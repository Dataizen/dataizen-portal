...
// Rapport de contrôle qualité d'un jeu (déterministe + suggestions IA), via dtz-rag.
...
export async function POST(request) {
  ...
  try {
    const r = await fetch(`${RAG}/quality`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Dtz-Token': process.env.RAG_WEBHOOK_TOKEN || '' },
      body: JSON.stringify({ dataset: name }),
      signal: AbortSignal.timeout(150000),
    });
    if (!r.ok) return NextResponse.json({ error: 'contrôle qualité indisponible' }, { status: 502 });
    return NextResponse.json(await r.json());
  } catch {
    return NextResponse.json({ error: 'contrôle qualité indisponible' }, { status: 502 });
  }
}
