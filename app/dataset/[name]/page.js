...
import { METADATA_FIELDS, extrasToObject, fieldLabel, optionLabelByValue } from '../../../lib/metadata';
        ...
        <dl className="fiche">
          ...
          {METADATA_FIELDS.filter((f) => extras[f.key] && !f.key.startsWith('validite_')).map((f) => (
            <div key={f.key} style={{ display: 'contents' }}>
              <dt>{fieldLabel(f, t)}</dt><dd>{f.i18nOptions ? optionLabelByValue(f, extras[f.key], t) : extras[f.key]}</dd>
            </div>
          ))}
          {extras.depose_par && (
