...
export async function previewDatastore(resourceId, limit = 10) {
  try {
    return await action('datastore_search', { resource_id: resourceId, limit });
  } catch {
...
export async function previewDatastorePrivileged(resourceId, limit = 10) {
  ...
  if (!token) return null;
  try {
    const url = `${CKAN}/api/3/action/datastore_search?resource_id=${encodeURIComponent(resourceId)}&limit=${limit}`;
