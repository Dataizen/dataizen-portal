...
import { themeSolrValues } from './metadata';
...
export async function searchDatasets({ q = '', organization = '', format = '', theme = '', tags = '', territoire = '', territoireRegion = '', perime = '', harmonise = '', utilise = '', orgs = null, start = 0, rows = 10 }) {
  ...
  // thème = code EU (data-theme). Filtre rétro-compatible : le code ET ses anciens
  // libellés FR (jeux non encore migrés), pour ne rien perdre pendant la transition.
  if (theme) {
    const vals = themeSolrValues(theme);
