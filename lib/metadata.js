...
export const METADATA_FIELDS = [
  ...
  { key: 'langue', label: 'Langue', type: 'select', i18nOptions: 'meta.o.langue',
    options: ['Français', 'Anglais', 'Multilingue', 'Autre'] },
  { key: 'theme', label: 'Thématique', type: 'select', options: THEME_CODES, i18nOptions: 'theme' },
  { key: 'type_donnees', label: 'Type de données', type: 'select', i18nOptions: 'meta.o.type',
  ...
  { key: 'validite_fin', label: 'Fin de validité (péremption)', type: 'date',
    placeholder: 'AAAA-MM-JJ' },
  { key: 'frequence', label: 'Fréquence de mise à jour', type: 'select', i18nOptions: 'meta.o.freq', options: FREQUENCES },
...
// Rendu localisé des champs de métadonnées. Le libellé vient de la clé i18n `meta.<key>`,
// le placeholder de `meta.<key>.ph`, les options d'un select de `i18nOptions.<index>` (ou
// `theme.<code>` pour le thème). Les valeurs STOCKÉES restent inchangées (français canonique
// pour langue/type/fréquence/dolfin) : seul l'affichage est traduit. `t` = fonction i18n
// (serveur `t` ou client `useT`). Repli sur le texte français d'origine si la clé manque.
export function fieldLabel(f, t) {
  const k = 'meta.' + f.key;
  const v = t(k);
  return v === k ? (f.label || f.key) : v;
}

export function fieldPlaceholder(f, t) {
  if (!f.placeholder) return '';
  const k = 'meta.' + f.key + '.ph';
  const v = t(k);
  return v === k ? f.placeholder : v;
}

export function optionLabel(f, value, index, t) {
  if (!f.i18nOptions) return value;
  if (f.i18nOptions === 'theme') { const v = t('theme.' + value); return v === 'theme.' + value ? value : v; }
  const k = f.i18nOptions + '.' + index;
  const v = t(k);
  return v === k ? value : v;
}

// Affichage d'une valeur de select stockée (français canonique) -> libellé localisé, par
// recherche de son index dans les options du champ. Repli sur la valeur brute.
export function optionLabelByValue(f, value, t) {
  if (f.i18nOptions === 'theme') return optionLabel(f, themeCode(value) || value, 0, t);
  const i = (f.options || []).indexOf(value);
  return i >= 0 ? optionLabel(f, value, i, t) : value;
}
