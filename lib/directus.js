  ...
  return html
    ...
    .replace(/\[\[youtube:([^\]]+)\]\]/g, (_, v) => {
      ...
      return `<div class="embed dtz-youtube"><iframe src="https://www.youtube-nocookie.com/embed/${id}" `
        + `title="Vidéo YouTube" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" `
...
const escHtml = (s) => String(s || '').replace(/&/g, '&amp;').replace(/</g, '&lt;')
  .replace(/>/g, '&gt;').replace(/"/g, '&quot;');  // " échappé : indispensable dans les attributs (ex. data-values JSON)
